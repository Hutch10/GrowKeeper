  ...
  const handleBulkFertilize = () => {
    startTransition(async () => {
      ...
      for (const task of tasksToFertilize) {
        try {
          await operationQueue.enqueue('fertilize', {
            taskId: task.id,
            specimenId: task.specimen_id,
          });
          queuedCount++;
        } catch (err) {
          console.error("Failed to queue fertilize action:", err);
        }
      }
      ...
      if (queuedCount > 0) {
        if (navigator.onLine) {
          toast.success(`Fertilizing ${queuedCount} specimen${queuedCount > 1 ? "s" : ""}...`);
        } else {
          toast.info(`Offline: Queued ${queuedCount} fertilization action${queuedCount > 1 ? "s" : ""} for sync.`);
        }
      }
